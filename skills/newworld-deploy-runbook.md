...
> **2026-07-24 BL-174：新迁移改由 Flyway 追踪**（人肉 ledger `sql/MIGRATIONS.md §4` 退役）。
> - 新迁移 = `V<n>__<snake_desc>.sql` 落 `newworld-admin/src/main/resources/db/migration/`（V1=存量 baseline，从 V2 起）。
> - **部署期**（提权 root socket / DBA，app 账号 `newworld` 无 ALTER）：
>   ```bash
>   export PATH=/opt/apache-maven-3.9.9/bin:$PATH
>   mvn -pl newworld-admin flyway:migrate \
>     -Dflyway.url='jdbc:mysql://<DB_HOST>:3306/newworld' \
>     -Dflyway.user=<提权账号> -Dflyway.password=<...>
>   ```
>   **不传 `-Dflyway.locations`**：plugin 已配 `filesystem:.../src/main/resources/db/migration`（绑 git 源）。禁用 `classpath:db/migration`——绑 `target/classes`：不 clean 应用残留旧迁移、clean 后不 compile 又解析为空静默 no-op（BLOCKER）。
>   `baseline-on-migrate=true` 对存量库只写 V1 baseline 记录、**跳过 version≤1 → 170 存量档零重放**（不炸生产）。
> - **admin 运行期**：prod `nw.flyway.auto-migrate=false` → 启动期只 `validate`（不跑 DDL）；**漏跑 / checksum 漂移 → 启动即红（fail-fast）**。dev/local=true → 直接 migrate。
> - **首次上线顺序**：先跑一次部署期 `flyway:migrate`（建 history 表 + baseline + 应用 V2+），**再上 validate 模式的 admin jar**；反序（先上 jar、history 表还没建）→ validate 抛 `FlywayValidateException`（history 缺失 / pending 未应用）→ 启动红（fail-safe，非静默），非事故但阻部署。（jar 漏带迁移资源致 classpath 解析 0 迁移的盲区由 `NwFlywayConfig` 的 resolved==0 guard 兜底。）
> - 大表 / 分区仍 **DBA-gated**：作为 V-文件在 DBA 窗口经部署期 `flyway:migrate` 应用，`out-of-order=false` 保证顺序。
> - 全新空库 bootstrap 仍走 `sql/import-all.sh`（Flyway 不从零重建整库）。
> - 遗留 `sql/*.sql`（非 db/migration）仍受 `check-sql-ledger.sh` 门管（该门只扫 sql/ 根，不碰 db/migration）；下方旧机制（deploy-backend.sh sql diff 门）对遗留 sql/ 仍有效、与 Flyway 并存不冲突。

**以下为遗留 sql/ 目录的历史机制**（Flyway 纳管前的兜底，对 db/migration 新迁移已由上方 Flyway 取代）：
