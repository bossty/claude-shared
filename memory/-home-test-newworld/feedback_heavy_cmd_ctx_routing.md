...
**Why:** 本条原版（2026-06-13）主张"默认走 ctx_batch_execute"，依据是单臂实测 137× 杠杆。2026-07-05 A/B 实验证伪：同批 4 条 ops 命令，纪律化 Bash 进会话 3,411 字节且答案齐全，ctx 路由 19,148 字节且 2/4 查询没答上（约 5.6× 差距）；插件自报节省以"裸吞全量"为假设，对照真实基线夸大 50~450×；历史提示注入成本（~69 万 token）超过其自报累计节省（516,608）。context-mode 插件已停用（`~/.claude-work/settings.json` enabledPlugins=false）。全文见 `docs/sprint/_archive/2026-07-05-context-mode-ab/POC-FINDINGS.md`。
