...
- ~~原生 systemd `EnvironmentFile=/etc/newworld/secrets.env` 显示已加载，但 nginx master 进程 env 读不到（疑似 fork 时丢失）~~ **★ 2026-07-22 实测订正：这条从来就不成立，是判据错不是机制错。**
  ...
  - 反向正证：04-25（本档写完仅 4 天后）wave-stats-v4 W2 sprint 在 aws-web-01/02 上用**纯 EnvironmentFile、无 wrapper** + `systemctl restart`，经 lua `os.getenv()` 反查拿到 `_G._ANALYTICS_V4_ENABLED=true` / `_G._EDGE_OPS_SECRET_len=64` / `_G._REDIS_PWD_len=20`（`docs/design/wave_stats_v4_sprint_closure.md:309-323`，commit `dcedf9d558c`）。同档 §6.4 已明写「验证 env 注入只能通过 lua 内反查 `_G._*` 或 `os.getenv()`」。
