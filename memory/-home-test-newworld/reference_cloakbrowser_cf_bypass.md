...
**★2026-07-12 CN 拨测源 POC 负结果（cloakbrowser 自动化这 4 个公开拨测源不可靠）**：用 cloakbrowser 0.4.10/Chromium146 驱动 4 个公开 CN 拨测源做 web 可达性探测（授权目标：baidu 对照 + tundramesa.top），**全部拿不到可信 target-bound 结果**——**Alibaba(boce.aliyun.com) INCONCLUSIVE**（两轮完成 582/541 行、31 省，但 provider 返回未绑定到请求 target，`control_target_binding_missing`，存的 target 只是请求侧 label 非 provider 回填）；**tcptest.cn FAIL**（验证码硬停）；**boce.com FAIL**（免费额度硬停）；**17ce.com INCONCLUSIVE**（locator-action 超时）。结论：这 4 个公开拨测源即便套 cloakbrowser 隐身也难做可信自动化（验证码/额度/结果绑定三类拦），**印证专用 headful `aliyun-probe-runner`(+Xvfb)+sec/sign 签名头才是正道，别再想直接爬这些公开源**。全文原在 `docs/superpowers/reports/2026-07-12-cn-web-probe-cloakbrowser-poc.md`（已删除留墓碑，`git show` 取回）。
