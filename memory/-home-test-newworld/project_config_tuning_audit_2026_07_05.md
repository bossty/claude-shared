...
**待办**：**批 D 维护窗另约**（skip_name_resolve 前置已核授权表零 hostname；EU replica relay_log_recovery+buffer pool 10G）；**07-11** binlog 压缩 SET PERSIST；**07-07 晚**紧凑对象头灰度 24h 评估（ca-web-02 vs 其余 jvm_memory_used）→达标全 fleet+评估 A5 字符串去重；A3 虚拟线程（Hikari 收敛跑稳后压测评估）；A4 S3 快照/A6 request_id/A7 ACL=P3；CA `background_snapshotting=true` 与 EU 差异未统一（非阻塞观察项）；新发现=CA web exec.conf 缺 --app.scheduling.enabled=false（两地行为差异待查 web 有无 @Scheduled）/ca-web-04 journald 无日志文件/base unit 残留退役 HK IP/EU cloudflared 三 unit 二进制路径不一致（a=/usr/bin, c,p=/usr/local/bin，已都升级但路径未标准化）。
